...
public class debugtes : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 1; i <= LightsCount; i++)
        {
            ...
            Debug.Log(LightPolarCoordinates);
            Debug.Log("label: "+i+" Polar :"+LightPolarCoordinates / YEl / Mathf.PI * 180f);
        ...
        {
            Vector3 position=Polar2Position(PolarCoordinates[i].x, PolarCoordinates[i].y,1);
            float intensity = LuminanceValue[i];
            Color color = IrradianceValue[i];
            CreateLight( position,color,Light,intensity,parent);
        }
        ...
        for (int x = 0; x < width; x++)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                if (labels[x+y*width] == 0 && IsAboveThreshold(InputTex[x + y * width]))
                {
                    componentCount++;
                    // labels配列もBFSに渡す
                    BFS(x, y, componentCount, InputTex, labels);
                }
            }
        }
    ...
    // labelsを引数に追加
    void BFS(int startX, int startY, int label, float[] pixels, int[] labels)
    {
